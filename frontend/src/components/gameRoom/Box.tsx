...
const Box: React.FC<BoxType> = ({ top, score, bottom, color }) => {
  return (
    <div
      className={`p-1 w-[80px] sm:w-24 h-full text-white flex flex-col items-center justify-center bg-gradient-radial rounded-lg leading-none ${
        color == 'orange'
          ? 'from-secondary-orange to-primary-orange'
          : color == 'blue'
          ? 'from-secondary-blue to bg-primary-blue'
